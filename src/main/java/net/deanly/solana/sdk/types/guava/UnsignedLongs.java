package net.deanly.solana.sdk.types.guava;
/*
...
public final class UnsignedLongs {
    ...
    /**
     * Ensures that {@code fromIndex} and {@code toIndex} represent a valid range within an array, list,
     * or string of size {@code size}.
     *
     * @param fromIndex the starting index (inclusive)
     * @param toIndex the ending index (exclusive)
     * @param size the total size of the array, list, or string
    ...
    private static void checkPositionIndexes(int fromIndex, int toIndex, int size) {
        if (fromIndex < 0 || toIndex < fromIndex || toIndex > size) {
            throw new IndexOutOfBoundsException(formatPositionIndexesMessage(fromIndex, toIndex, size));
        }
    }

    /**
     * Produces a formatted error message for invalid position indexes.
     */
