...
/**
 * Adds liquidity to a CPMM pool (Deposit).
 *
 * <pre>
 * Accounts expected:
 * 0. `[signer]` The depositor's account.
...
@AllArgsConstructor
@ToString
public class CpmmInstruction01Deposit extends RaydiumCpmmProgram.Base implements TransactionInstruction {

    @StructField(order = 1, type = Bytes8Field.class)
    private final byte[] discriminator = new byte[] {
            (byte) 242, (byte) 35, (byte) 198, (byte) 137, (byte) 82, (byte) 225, (byte) 242, (byte) 182
    }; // Discriminator for Deposit (index 1)

    @Setter
    @StructField(order = 2, type = UInt64LEField.class)
    private long lpAmount; // The amount of LP tokens to receive.
    ...
    /**
     * Sets the account metadata (keys) for the Deposit instruction.
     *
     * @param depositor        The depositor's account (signer).
    ...
    public void setKeys(
        ...
        if (depositor == null || authority == null || poolId == null || userLpAccount == null ||
        ...
        this.keys.add(new AccountMeta(depositor, true, false)); // Depositor: signer, not writable
    ...
    @Override
    public byte[] getData() {
        // Encodes the discriminator, lpAmount, amountMaxA, and amountMaxB.
        return StructLayout.encode(this);
    }

    public void setData(byte[] data) {
        // Decode fields (discriminator, lpAmount, amountMaxA, amountMaxB). Keys must be set explicitly.
        CpmmInstruction01Deposit decoded = StructLayout.decode(data, CpmmInstruction01Deposit.class);
    ...
    /**
     * Static factory method to create and configure a Deposit instruction.
     *
     * @param depositor        The depositor's account (signer).
     * @param authority        The pool authority account.
     * @param poolId           The pool account (writable).
     ...
     * @param mintB            Token B mint account.
     * @param lpMint           LP token mint account (writable).
     * @param lpAmount         The amount of LP tokens to receive.
     * @param amountMaxA       Maximum amount of Token A allowed.
     * @param amountMaxB       Maximum amount of Token B allowed.
     * @return A fully configured CpmmInstruction01Deposit instance.
     */
    public static CpmmInstruction01Deposit create(
            PublicKey depositor,
            PublicKey authority,
            PublicKey poolId,
            PublicKey userLpAccount,
        ...
        CpmmInstruction01Deposit instruction = new CpmmInstruction01Deposit();
        instruction.setLpAmount(lpAmount);
        instruction.setAmountMaxA(amountMaxA);
        instruction.setAmountMaxB(amountMaxB);
        instruction.setKeys(depositor, authority, poolId, userLpAccount, userVaultA, userVaultB, vaultA, vaultB, tokenProgram, mintA, mintB, lpMint);
        return instruction;
    }
}