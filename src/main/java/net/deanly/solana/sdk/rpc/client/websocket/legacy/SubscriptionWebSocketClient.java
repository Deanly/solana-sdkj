...
public class SubscriptionWebSocketClient extends WebSocketClient {
    ...
    public void addSubscription(RpcRequest rpcRequest, NotificationEventListener listener) {
        String subscriptionId = String.valueOf(rpcRequest.getId());
        subscriptionLock.lock();
        try {
    ...
    public void onMessage(String message) {
        try {
            JsonAdapter<RpcResponse<RpcResultObject<Long>>> resultAdapter = moshi.adapter(
                    Types.newParameterizedType(RpcResponse.class, Long.class));
            RpcResponse<RpcResultObject<Long>> rpcResult = resultAdapter.fromJson(message);
     ...
     */
    private void handleSubscriptionResponse(RpcResponse<RpcResultObject<Long>> rpcResult) {
        String rpcResultId = String.valueOf(rpcResult.getId());
        if (subscriptionIds.containsKey(rpcResultId)) {
            subscriptionIds.put(rpcResultId, rpcResult.getResult().getValue());
            SubscriptionParams params = subscriptions.get(rpcResultId);
            if (params != null) {
                subscriptionListeners.put(rpcResult.getResult().getValue(), params.listener);
    ...
    private void resubscribeAll() {
        ...
        for (Map.Entry<String, SubscriptionParams> entry : activeSubscriptions.entrySet()) {
            ...
            final String subscriptionId = String.valueOf(request.getId());
